...
export interface VMessage extends Error {
  /**
  ...
  column: number;
  /**
   * Full range information, when available (Position). Has start and end properties, both set to an object with line
   * and column, set to number?.
   */
  ...
  ruleId: string;
  /**
   * If true, marks associated file as no longer processable. If false, necessitates a (potential) change. The value
...
interface VFile {
  ...
  cwd: string;
  /**
   * Current name (including extension) of vfile. Cannot contain path separators. Cannot be nullified either
   * (use file.path = file.dirname instead).
   */
  basename: string;
  /**
  ...
  messages: VMessage[];
  /**
   * Place to store custom information. It’s OK to store custom data directly on the vfile, moving it to data
   * gives a little more privacy.
   */
  data: string;
}

export interface IPrettyHtml {
  (
    input: string,
    options?: {
      tabWidth?: number;
