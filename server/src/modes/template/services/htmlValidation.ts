...
function toDiagnostic(error: ESLintError): Diagnostic {
  ...
  const endLine = error.endLine != null ? error.endLine - 1 : line;
  const endColumn = error.endColumn != null ? error.endColumn - 1 : column;
  return {
...
export function doValidation(document: TextDocument, engine: CLIEngine): Diagnostic[] {
  ...
  return report.results[0].messages.map(toDiagnostic);
}
