...
const routerTags = {
  'router-link': new HTMLTagSpecification(
    ...
    [ genAttribute('to', u, 'The target route of the link. It can be either a string or a location descriptor object.'),
      genAttribute('replace', u, 'Setting replace prop will call router.replace() instead of router.push() when clicked, so the navigation will not leave a history record.'),
      genAttribute('append', u, 'Setting append prop always appends the relative path to the current path. For example, assuming we are navigating from /a to a relative link b, without append we will end up at /b, but with append we will end up at /a/b.'),
      ...
      genAttribute('exact-active-class', u, 'Configure the active CSS class applied when the link is active with exact match.')
    ]
  ),
